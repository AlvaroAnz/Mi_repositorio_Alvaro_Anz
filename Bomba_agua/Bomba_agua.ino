...
#define led_azul 4 //indica que esta vacío
#define led_rojo 5 //indica que esta lleno
...
void loop() {
  ...
  if (limite ==  HIGH) { // si el sensor no recibe señal
    encender(rele_bomba);
    encender(led_azul);
    apagar(led_rojo);
  }

  if (limite ==  LOW) { // si el sensor recibe señal (el transistor pasa a corte y saturación)
    apagar(rele_bomba);
    apagar(led_azul);
    encender(led_rojo);
  }
